...
//NEW// Keyframes
float posXavion = 0.0, posYavion = 2.0, posZavion = 0;
...
typedef struct _frame
{
	...
	float rotAla;           //Variable para RotacionAla

	float movAvion_xInc;	//Variable para IncrementoX
	float movAvion_yInc;	//Variable para IncrementoY
	float giroAvionInc;     //Variable para IncrementoGiro

	float rotAlaInc;        //Variable para IncrementoRotacionAla
...
void saveFrame(void) //tecla L
{
	...
	KeyFrame[FrameIndex].rotAla = rotAla; //Guardar la rotación del ala en el keyframe
...
void resetElements(void) //Tecla 0
{
	...
	rotAla = KeyFrame[0].rotAla; //Reiniciar la rotación del ala al valor del primer keyframe
}
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].rotAlaInc = (KeyFrame[playIndex + 1].rotAla - KeyFrame[playIndex].rotAla) / i_max_steps;
}
...
void animate(void)
{
	...
	if (play)
	{
		...
		else
		{
			...
			rotAla += KeyFrame[playIndex].rotAlaInc;
			i_curr_steps++;
		}
...
int main()
{
	...
	KeyFrame[0].rotAla = 25.0f;

	//Subiendo a cresta 1 (x=-3, y=1.5)
	KeyFrame[1].movAvion_x = -3.0f;
	KeyFrame[1].movAvion_y = 1.5f;
	KeyFrame[1].giroAvion = 0.0f;
	KeyFrame[1].rotAla = -25.0f;

	//Bajando a valle 1 (x=-1, y=-1.5)
	KeyFrame[2].movAvion_x = -1.0f;
	KeyFrame[2].movAvion_y = -1.5f;
	KeyFrame[2].giroAvion = 0.0f;
	KeyFrame[2].rotAla = 25.0f;

	//Subiendo a cresta 2 (x=1, y=1.5)
	KeyFrame[3].movAvion_x = 1.0f;
	KeyFrame[3].movAvion_y = 1.5f;
	KeyFrame[3].giroAvion = 0.0f;
	KeyFrame[3].rotAla = -25.0f;

	//Bajando a valle 2 (x=3, y=-1.5)
	KeyFrame[4].movAvion_x = 3.0f;
	KeyFrame[4].movAvion_y = -1.5f;
	KeyFrame[4].giroAvion = 0.0f;
	KeyFrame[4].rotAla = 25.0f;

	//Llegando al extremo derecho (x=5, y=0) y gira
	KeyFrame[5].movAvion_x = 5.0f;
	KeyFrame[5].movAvion_y = 0.0f;
	KeyFrame[5].giroAvion = 180.0f;   // ¡Gira 180 grados!
	KeyFrame[5].rotAla = -25.0f;

	//************* MOVIMIENTO DE VUELTA (Regreso) *************** 
	// Punto 6: Subiendo a cresta 2 (de regreso)
	KeyFrame[6].movAvion_x = 3.0f;
	KeyFrame[6].movAvion_y = 1.5f;
	KeyFrame[6].giroAvion = 180.0f;
	KeyFrame[6].rotAla = 25.0f;

	//Bajando a valle 2 (de regreso)
	KeyFrame[7].movAvion_x = 1.0f;
	KeyFrame[7].movAvion_y = -1.5f;
	KeyFrame[7].giroAvion = 180.0f;
	KeyFrame[7].rotAla = -25.0f;

	//Subiendo a cresta 1 (de regreso)
	KeyFrame[8].movAvion_x = -1.0f;
	KeyFrame[8].movAvion_y = 1.5f;
	KeyFrame[8].giroAvion = 180.0f;
	KeyFrame[8].rotAla = 25.0f;

	//Bajando a valle 1 (de regreso)
	KeyFrame[9].movAvion_x = -3.0f;
	KeyFrame[9].movAvion_y = -1.5f;
	KeyFrame[9].giroAvion = 180.0f;
	KeyFrame[9].rotAla = -25.0f;

	//Regreso al inicio (x=-5, y=0)
	KeyFrame[10].movAvion_x = -5.0f;
	KeyFrame[10].movAvion_y = 0.0f;
	KeyFrame[10].giroAvion = 0.0f;     //Vuelve a orientación original
	KeyFrame[10].rotAla = 25.0f;

	i_max_steps = 60; //Se ajusta la velocidad (más pasos = más lento)
	*/

	FrameIndex = 0;  //0 keyframes
...
void inputKeyframes(bool* keys)
{
	if (keys[GLFW_KEY_SPACE])
	{
		if (reproduciranimacion < 1)
		{
			...
			{
				resetElements();
				rotAla = KeyFrame[0].rotAla; //Reiniciar posición de ala
				//First Interpolation				
				interpolation();
				play = true;
				playIndex = 0;
	...
	//Mover nave con las flechas del teclado
	
	if (keys[GLFW_KEY_RIGHT]) 
	{
		movAvion_x += 0.5f;
		printf("X: %.1f, Y: %.1f, Giro: %.0f, Ala: %.1f\n", movAvion_x, movAvion_y, giroAvion, rotAla);
	}
	...
	{
		giroAvion += 90.0f;
		if (giroAvion >= 360) giroAvion -= 360;
		printf("X: %.1f, Y: %.1f, Giro: %.0f, Ala: %.1f\n", movAvion_x, movAvion_y, giroAvion, rotAla);
	}

	//Control del ángulo de las alas (tecla Z)
	if (keys[GLFW_KEY_Z]) 
	{
		rotAla += 15.0f;
		if (rotAla > 25.0f) rotAla = -25.0f;
		printf("Ala: %.1f grados\n", rotAla);
	}
